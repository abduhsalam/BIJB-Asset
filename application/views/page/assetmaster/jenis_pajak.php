...
<div class="row-fluid">
	<div class="span12">
		<div class="span5">
			<div class="row-fluid">
				<div class="span12">
					<form id="form-jns-pajak" class="form-horizontal" /> 
		
						<div class="control-group">
							<label class="control-label span4" for="jnsPajak">Jenis Pajak</label>
							<div class="controls span8">
								<input id="jnsPajak" type="text" name="jnsPajak" class="uppercase"> 
							</div> 
						</div> 
		...
		<div class="span7">	 			
			<table class="table table-striped table-bordered" id="dt-list-jnsPajak">
				<thead>
					<tr>
				...
				<tbody id="">						
					...
					<?php 
					$no = 1;
					foreach($dtJnsPajak as $val): ?>
					<tr id="">
						<td class="center"><?= $no++; ?></td>
						<td class="center td2"><?= $val->kode_jnspajak; ?></td>
						<td class="center td3"><?= $val->nama_jnspajak; ?></td> 
						<td class="center">
...
<script type="text/javascript">
	$(document).ready(function () { 
		...
		var dataTable = $('#dt-list-jnsPajak').dataTable( {
			// bAutoWidth: false,
			"aoColumns": [    
			  { "bSortable": false }, 
			  { "bSortable": false }, 
		...
		$('#simpan').click(function(){  
			var jnsPajak = $('#jnsPajak').val();  
			if(jnsPajak!=""){
				var cek = cekjnsPajak();
				if(cek<1){
					$('#modalLoading').modal('show');
					$.ajax({
					url: '<?php echo base_url(); ?>Assetmaster/tambah_jnsPajak',
					data: {"jnsPajak":jnsPajak}, 
					type: 'post',
					dataType:'json',
		...
		$('#dt-list-jnsPajak').on('click','.btn-edit',function(){
			var kode 	 	= $(this).closest('tr').find('.td2').text(); 
			var jnsPajak= $(this).closest('tr').find('.td3').text(); 
			$('#jnsPajak').val(jnsPajak);  
			$('#update').val(kode);
			$('#update').show();
			$('#simpan').hide();
		});
		...
		$('#update').click(function(){  
			...
			$('#modalLoading').modal('show');
			$.ajax({
			url: '<?php echo base_url(); ?>Assetmaster/update_jnspajak',
			data: {'kode':kode, 'jnsPajak':jnsPajak}, 
			type: 'post',
			dataType:'json',
			success: function(data){	
					$('#modalLoading').modal('hide');
		...
		$('#delete-action').click(function(){ 
			...
			$('#modalLoading').modal('show');
			$.ajax({
			url: '<?php echo base_url(); ?>Assetmaster/delete_jnspajak',
			data: {'kode':kode}, 
			type: 'post',
			dataType:'json',
			success: function(data){	
		...
		$('#dt-list-jnsPajak').on('click','.btn-delete',function(){ 
			var kode 	 	= $(this).closest('tr').find('.td2').text(); 
	...
	function cekjnsPajak(){
		var jnsPajak = $('#jnsPajak').val();
		var cek =0;
		$.ajax({
		url: '<?php echo base_url(); ?>Assetmaster/cek_jnspajak',
		data: {'jnsPajak':jnsPajak}, 
		type: 'post',
		dataType:'json',
		async:false,
	...
	function reload_page(){
		...
		var action=''; 
		form.method = "POST"; 
		action = 'Assetmaster/jenis_pajak';
