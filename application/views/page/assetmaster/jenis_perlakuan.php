...
<div class="row-fluid">
	<div class="span12">
				...
				<div class="span12">
					<form id="form-jns-perlakuan" class="form-horizontal" /> 
						...
						<div class="control-group">
							<label class="control-label span4" for="jnsPerlakuan">Jenis Perlakuan</label>
							<div class="controls span8">
								<input id="jnsPerlakuan" type="text" name="jnsPerlakuan" class="uppercase"> 
							</div> 
						</div> 
		...
		<div class="span7">	 			
			<table class="table table-striped table-bordered" id="dt-list-jnsperlakuan">
				<thead>
					<tr>
				...
				<tbody id="">						
					...
					<?php 
					$no = 1;
					foreach($dtJnsPerlakuan as $val): ?>
					<tr id="">
						<td class="center"><?= $no++; ?></td>
						<td class="center td2"><?= $val->kode_jnsperlakuan; ?></td>
						<td class="center td3"><?= $val->nama_jnsperlakuan; ?></td> 
						<td class="center">
...
<script type="text/javascript">
	$(document).ready(function () { 
		...
		var dataTable = $('#dt-list-jnsperlakuan').dataTable( {
			// bAutoWidth: false,
			"aoColumns": [    
			  { "bSortable": false }, 
			  { "bSortable": false }, 
		...
		$('#simpan').click(function(){  
			var jnsPerlakuan = $('#jnsPerlakuan').val();  
			if(jnsPerlakuan!=""){
				var cek = cekJnsPerlakuan();
				if(cek<1){
					$('#modalLoading').modal('show');
					$.ajax({
					url: '<?php echo base_url(); ?>Assetmaster/tambah_jnsperlakuan',
					data: {"jnsPerlakuan":jnsPerlakuan}, 
					type: 'post',
					dataType:'json',
				...
				$.gritter.add({
					title: 'Informasi',
					text: 'Silahkan isi dulu jenis perlakuan',
					class_name: 'gritter-error gritter-center'
				});  
				$('#jnsperlakuan').focus();
			}
		...
		$('#dt-list-jnsperlakuan').on('click','.btn-edit',function(){
			var kode 	 	= $(this).closest('tr').find('.td2').text(); 
			var jnsPerlakuan= $(this).closest('tr').find('.td3').text(); 
			$('#jnsPerlakuan').val(jnsPerlakuan);  
			$('#update').val(kode);
			$('#update').show();
			$('#simpan').hide();
		});
		...
		$('#update').click(function(){  
			...
			$('#modalLoading').modal('show');
			$.ajax({
			url: '<?php echo base_url(); ?>Assetmaster/update_jnsperlakuan',
			data: {'kode':kode, 'jnsPerlakuan':jnsPerlakuan}, 
			type: 'post',
			dataType:'json',
			success: function(data){	
					$('#modalLoading').modal('hide');
		...
		$('#delete-action').click(function(){ 
			...
			$('#modalLoading').modal('show');
			$.ajax({
			url: '<?php echo base_url(); ?>Assetmaster/delete_jnsperlakuan',
			data: {'kode':kode}, 
			type: 'post',
			dataType:'json',
			success: function(data){	
		...
		$('#dt-list-jnsperlakuan').on('click','.btn-delete',function(){ 
			var kode 	 	= $(this).closest('tr').find('.td2').text(); 
	...
	function cekJnsPerlakuan(){
		var jnsperlakuan = $('#jnsperlakuan').val();
		var cek =0;
		$.ajax({
		url: '<?php echo base_url(); ?>Assetmaster/cek_jnsperlakuan',
		data: {'jnsperlakuan':jnsperlakuan}, 
		type: 'post',
		dataType:'json',
		async:false,
	...
	function reload_page(){
		...
		var action=''; 
		form.method = "POST"; 
		action = 'Assetmaster/jenis_perlakuan';
		 
		form.action = "<?php echo base_url(); ?>" + action,
		 
		
		input.value = '';
		input.name = "jnsperlakuan";
		input.type = "hidden";
