...
<div class="row-fluid">
	<div class="span12">
				...
				<div class="span12">
					<form id="form-sbr-anggaran" class="form-horizontal" /> 
						...
						<div class="control-group">
							<label class="control-label span4" for="namaAnggaran">Nama Anggaran</label>
							<div class="controls span8">
								<input id="namaAnggaran" type="text" name="namaAnggaran" class="uppercase"> 
							</div> 
						</div> 
		...
		<div class="span7">	 			
			<table class="table table-striped table-bordered" id="dt-list-sbranggaran">
				<thead>
					<tr>
						<th class="center">No</th>  
						<th class="center">Kode</th>  
						<th class="center">Sumber Anggaran</th>   
						<th class="center">#</th>   
					</tr>
				</thead>
				...
				<tbody id="">						
					...
					<?php 
					$no = 1;
					foreach($dtSbrAnggaran as $val): ?>
					<tr id="">
						<td class="center"><?= $no++; ?></td>
						<td class="center td2"><?= $val->kode_sumberanggaran; ?></td>
						<td class="center td3"><?= $val->nama_sumberanggaran; ?></td> 
						<td class="center">
...
<script type="text/javascript">
	$(document).ready(function () { 
		...
		var dataTable = $('#dt-list-sbranggaran').dataTable( {
			// bAutoWidth: false,
			"aoColumns": [    
			  { "bSortable": false }, 
			  { "bSortable": false }, 
		...
		$('#simpan').click(function(){  
			var sbrAnggaran = $('#namaAnggaran').val();  
			if(sbrAnggaran!=""){
				var cek = cekSbrAnggaran();
				if(cek<1){
					$('#modalLoading').modal('show');
					$.ajax({
					url: '<?php echo base_url(); ?>Assetmaster/tambah_sbranggaran',
					data: {"sbrAnggaran":sbrAnggaran}, 
					type: 'post',
					dataType:'json',
		...
		$('#dt-list-sbranggaran').on('click','.btn-edit',function(){
			var kode 	 	= $(this).closest('tr').find('.td2').text(); 
			var sbrAnggaran	= $(this).closest('tr').find('.td3').text(); 
			$('#namaAnggaran').val(sbrAnggaran);  
			$('#update').val(kode);
			$('#update').show();
			$('#simpan').hide();
		});
		...
		$('#update').click(function(){  
			...
			$('#modalLoading').modal('show');
			$.ajax({
			url: '<?php echo base_url(); ?>Assetmaster/update_sbranggaran',
			data: {'kode':kode, 'sbrAnggaran':sbrAnggaran}, 
			type: 'post',
			dataType:'json',
			success: function(data){	
					$('#modalLoading').modal('hide');
		...
		$('#delete-action').click(function(){ 
			...
			$('#modalLoading').modal('show');
			$.ajax({
			url: '<?php echo base_url(); ?>Assetmaster/delete_sbranggaran',
			data: {'kode':kode}, 
			type: 'post',
			dataType:'json',
			success: function(data){	
		...
		$('#dt-list-sbranggaran').on('click','.btn-delete',function(){
			var kode 	 	= $(this).closest('tr').find('.td2').text(); 
	...
	function cekSbrAnggaran(){
		var namaAnggaran = $('#namaAnggaran').val();
		var cek =0;
		$.ajax({
		url: '<?php echo base_url(); ?>Assetmaster/cek_sbranggaran',
		data: {'namaAnggaran':namaAnggaran}, 
		type: 'post',
		dataType:'json',
		async:false,
		success: function(data){	 
	...
	function reload_page(){
		...
		var action=''; 
		form.method = "POST"; 
		action = 'Assetmaster/sumber_anggaran';
		 
		form.action = "<?php echo base_url(); ?>" + action,
		 
		
		input.value = '';
		input.name = "sbranggaran";
		input.type = "hidden";
